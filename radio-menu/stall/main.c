...
typedef struct _IBusRadiomenuStallEngine IBusRadiomenuStallEngine;
typedef struct _IBusRadiomenuStallEngineClass IBusRadiomenuStallEngineClass;

struct _IBusRadiomenuStallEngine {
  IBusEngine parent;
};

struct _IBusRadiomenuStallEngineClass {
  IBusEngineClass parent;
};

static void ibus_radiomenustall_engine_class_init(IBusRadiomenuStallEngineClass	*klass);
static void ibus_radiomenustall_engine_init(IBusRadiomenuStallEngine *engine);
static void ibus_radiomenustall_engine_destroy(IBusRadiomenuStallEngine *engine);
...
G_DEFINE_TYPE(IBusRadiomenuStallEngine, ibus_radiomenustall_engine, IBUS_TYPE_ENGINE)

#define IBUS_TYPE_RADIOMENUSTALL_ENGINE (ibus_radiomenustall_engine_get_type())

GType ibus_radiomenustall_engine_get_type(void);

static void
ibus_radiomenustall_engine_class_init(IBusRadiomenuStallEngineClass *klass)
{
  g_debug(G_STRFUNC);
  IBusObjectClass *ibus_object_class = IBUS_OBJECT_CLASS (klass);
  IBusEngineClass *engine_class = IBUS_ENGINE_CLASS (klass);

  ibus_object_class->destroy = (IBusObjectDestroyFunc)ibus_radiomenustall_engine_destroy;
...
static void
ibus_radiomenustall_engine_init(IBusRadiomenuStallEngine *engine)
{
  g_debug(G_STRFUNC);
  IBusText *label = ibus_text_new_from_static_string("Radio Menu Stall");
  ...
  label = ibus_text_new_from_static_string("Radio Menu Stall Test");
...
static void
ibus_radiomenustall_engine_destroy(IBusRadiomenuStallEngine *engine)
{
  g_debug(G_STRFUNC);
  if (menu) {
    g_object_unref(menu);
...
{
  GError *error = NULL;
  GOptionContext *context = g_option_context_new("- IBus RadiomenuStall engine");
  ...
  ibus_factory_add_engine(factory, "radiomenustall", IBUS_TYPE_RADIOMENUSTALL_ENGINE);

  if (execute_by_ibus) {
    ibus_bus_request_name(bus, "com.example.IBus.RADIOMENUSTALL", 0);
  } else {
    IBusComponent *component = ibus_component_new("com.example.IBus.RADIOMENUSTALL",
                                                  "Radio Menu",
                                                  "1.0",
                                                  "MIT",
                                                  "Kentaro Hayashi",
                                                  ...
                                                  "",
                                                  "ibus-engine-radiomenustall");
    IBusEngineDesc *description = ibus_engine_desc_new("radiomenustall",
                                                       "RadiomenuStall",
