...
typedef struct _IBusRadioMenuNoNeedToUpdateEngine IBusRadioMenuNoNeedToUpdateEngine;
typedef struct _IBusRadioMenuNoNeedToUpdateEngineClass IBusRadioMenuNoNeedToUpdateEngineClass;

struct _IBusRadioMenuNoNeedToUpdateEngine {
  IBusEngine parent;
};

struct _IBusRadioMenuNoNeedToUpdateEngineClass {
  IBusEngineClass parent;
};

static void ibus_radiomenunoneedtoupdate_engine_class_init(IBusRadioMenuNoNeedToUpdateEngineClass	*klass);
static void ibus_radiomenunoneedtoupdate_engine_init(IBusRadioMenuNoNeedToUpdateEngine *engine);
static void ibus_radiomenunoneedtoupdate_engine_destroy(IBusRadioMenuNoNeedToUpdateEngine *engine);
...
G_DEFINE_TYPE(IBusRadioMenuNoNeedToUpdateEngine, ibus_radiomenunoneedtoupdate_engine, IBUS_TYPE_ENGINE)

#define IBUS_TYPE_RADIOMENUNONEEDTOUPDATE_ENGINE (ibus_radiomenunoneedtoupdate_engine_get_type())

GType ibus_radiomenunoneedtoupdate_engine_get_type(void);

static void
ibus_radiomenunoneedtoupdate_engine_class_init(IBusRadioMenuNoNeedToUpdateEngineClass *klass)
{
  g_debug(G_STRFUNC);
  IBusObjectClass *ibus_object_class = IBUS_OBJECT_CLASS (klass);
  IBusEngineClass *engine_class = IBUS_ENGINE_CLASS (klass);

  ibus_object_class->destroy = (IBusObjectDestroyFunc)ibus_radiomenunoneedtoupdate_engine_destroy;
...
static void
ibus_radiomenunoneedtoupdate_engine_init(IBusRadioMenuNoNeedToUpdateEngine *engine)
{
  g_debug(G_STRFUNC);
  IBusText *label = ibus_text_new_from_static_string("Radio Menu No Need To Update");
  ...
  label = ibus_text_new_from_static_string("Radio Menu No Need To Update Test");
...
static void
ibus_radiomenunoneedtoupdate_engine_destroy(IBusRadioMenuNoNeedToUpdateEngine *engine)
{
  g_debug(G_STRFUNC);
  if (menu) {
    g_object_unref(menu);
  ...
  ((IBusObjectClass *)ibus_radiomenunoneedtoupdate_engine_parent_class)->destroy((IBusObject *)engine);
}
...
{
  GError *error = NULL;
  GOptionContext *context = g_option_context_new("- IBus RadioMenuNoNeedToUpdate engine");
  ...
  ibus_factory_add_engine(factory, "radiomenunoneedtoupdate", IBUS_TYPE_RADIOMENUNONEEDTOUPDATE_ENGINE);

  if (execute_by_ibus) {
    ibus_bus_request_name(bus, "com.example.IBus.RADIOMENUNONEEDTOUPDATE", 0);
  } else {
    IBusComponent *component = ibus_component_new("com.example.IBus.RADIOMENUNONEEDTOUPDATE",
                                                  "Radio Menu",
                                                  "1.0",
                                                  "MIT",
                                                  "Kentaro Hayashi",
                                                  ...
                                                  "",
                                                  "ibus-engine-radiomenunoneedtoupdate");
    IBusEngineDesc *description = ibus_engine_desc_new("radiomenunoneedtoupdate",
                                                       "RadioMenuNoNeedToUpdate",
                                                       "Radio Menu No Need To Update (Japanese Input Method)",
                                                       "ja",
                                                       "MIT",
                                                       "Kentaro Hayashi",
                                                       "",
